...
async function loadCaseDetail(caseId) {
  try {
    ...
    const skinImg   = bestSkin.image_url || '';

    // Hero background: blurred best skin + rarity glow
    const bgEl = document.getElementById('open-hero-bg');
    bgEl.innerHTML = `
      <div style="position:absolute;inset:-50px;background:url(${skinImg}) center/cover no-repeat;filter:blur(48px) brightness(0.18) saturate(2.4);"></div>
